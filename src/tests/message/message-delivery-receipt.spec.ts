...
test('message_delivery_receipt webhook fires when a recipient client acknowledges delivery', async () => {
  ...
  const payload = await expectWebhookEvent('message_delivery_receipt', matchers.byReceiptMessageId(message.id), RECEIPT_TIMEOUT_MS);

  validateReceipt(payload, { trigger: 'message_delivery_receipt', action: 'delivered', messageId: message.id, recipient: QA_USER_2, sender: QA_USER_1 });
});
